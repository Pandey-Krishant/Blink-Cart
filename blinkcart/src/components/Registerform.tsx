...
import { 
  ShoppingCart, User, Mail, Lock, Eye, EyeOff, ArrowLeft, 
  Truck, Shirt, Apple, Package, Egg, Milk, Croissant, Carrot,
  Loader2 // Ek loader icon spin karne ke liye
} from "lucide-react";
...
export default function RegisterForm({ previousstep }: propType) {
  ...
  const isFormValid = name.trim() !== "" && email.includes("@") && password.length >= 8;
  ...
  return (
    <main className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#0a0a1a] font-sans">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        {/* Balanced Items (Balanced Opacity) */}
      ...
      <motion.div
        ...
        className="relative z-10 w-[95%] max-w-lg p-10 md:p-12 rounded-[3rem] border border-white/10 bg-white/[0.03] backdrop-blur-[45px] shadow-2xl mt-10 mb-10"
      >
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          <div className="space-y-2">
            <label className="flex items-center gap-2 text-sm font-semibold text-blue-400 ml-1"><User size={16} /> Full Name</label>
            <input value={name} onChange={(e) => setName(e.target.value)} type="text" placeholder="Your Name" className="w-full bg-white/[0.05] border border-white/10 rounded-2xl py-4 px-6 text-white outline-none focus:border-blue-500 transition-all" required />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input value={password} onChange={(e) => setPassword(e.target.value)} type={showpassword ? "text" : "password"} placeholder="Min 8 characters" className="w-full bg-white/[0.05] border border-white/10 rounded-2xl py-4 px-6 text-white outline-none focus:border-blue-500 transition-all" required />
          ...
          onClick={() => router.push("/login")} 
            ...
            className={`w-full py-4 mt-2 rounded-2xl font-bold text-lg flex items-center justify-center gap-2 transition-all duration-300 
              ${isFormValid && !isLoading 
                ? "bg-gradient-to-r from-purple-600 to-blue-500 text-white opacity-100 shadow-xl" 
